...
public class AppointmentDAO implements IAppointmentDAO {
	...
	public List<Appointment> searchAppointmentsByPhoneNumber(String phoneNumber) {
		...
		String hql = "SELECT a.appointmentId,a.owner.memberId, a.pet.petId, a.appointmentDate, "
				...
				+ "STRING_AGG(sp.packageDescription, ', ') as additionalPackages " + "FROM Appointment a "
				+ "LEFT JOIN a.owner o " + "LEFT JOIN a.pet p " + "LEFT JOIN ItemDetails id ON id.appointment = a "
				+ "LEFT JOIN Items i ON i = id.item " + "LEFT JOIN PackageDetails pd ON pd.appointment = a "
	...
	public List<Appointment> getAllAppointments() {
		...
		String hql = "SELECT a.appointmentId, a.owner.memberId, a.pet.petId, a.appointmentDate, "
				...
				+ "STRING_AGG(i.itemName, ', ') as serviceNames, "
				+ "STRING_AGG(sp.packageDescription, ', ') as additionalPackages " + "FROM Appointment a "
				+ "LEFT JOIN ItemDetails id ON id.appointment = a " + "LEFT JOIN Items i ON i = id.item "
		...
		try (Session session = sessionFactory.openSession()) {
			...
			for (Object[] row : result) {
				...
				appointment
						.setAdditionalPackages(StringUtils.isBlank(additionalPackages) ? "無加購服務" : additionalPackages);
				appointments.add(appointment);
			}
		}
		return appointments;
	...
	public ItemDetails getServiceById(int appointmentId) {
		...
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			Query<ItemDetails> query = session.createQuery("FROM ItemDetails WHERE appointmentId = :appointmentId",
					ItemDetails.class);
			query.setParameter("appointmentId", appointmentId);
